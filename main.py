...
def demo_pagination(teacher_manager):
    """Демонстрация постраничного просмотра"""
    page_size = 2
    total_teachers = teacher_manager.get_count()
    total_pages = (total_teachers + page_size - 1) // page_size

    for page in range(1, total_pages + 1):
        page_teachers = teacher_manager.get_k_n_short_list(page_size, page)
        display_short_teachers(page_teachers, f"Страница {page} из {total_pages}")

def demo_interactive_pagination(teacher_manager):
...
def main():
    ...
    k = 3  # количество элементов на странице
    n = 1  # номер страницы
    ...
    demo_interactive_pagination(teacher_manager)
